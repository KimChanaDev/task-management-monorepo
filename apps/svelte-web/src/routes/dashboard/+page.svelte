...
<div class="space-y-4 sm:space-y-6">
	...
	<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 md:gap-6">
		<StatLabel
			...
			head="Total Tasks"
			value={$taskStore.dashboard.stats.totalTasks}
			color="indigo"
		></StatLabel>
		<StatLabel
			d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
			head="To do"
			value={$taskStore.dashboard.stats.todoTasks}
			color="gray"
		></StatLabel>
		<StatLabel
			d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"
			head="In Progress"
			value={$taskStore.dashboard.stats.inProgressTasks}
			color="blue"
		></StatLabel>
		<StatLabel
			d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"
			head="Completed"
			value={$taskStore.dashboard.stats.completedTasks}
			color="green"
		></StatLabel>
		<StatLabel
			d="M10.125 2.25h-4.5c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125v-9M10.125 2.25h.375a9 9 0 0 1 9 9v.375M10.125 2.25A3.375 3.375 0 0 1 13.5 5.625v1.5c0 .621.504 1.125 1.125 1.125h1.5a3.375 3.375 0 0 1 3.375 3.375M9 15l2.25 2.25L15 12"
			head="Review"
			value={$taskStore.dashboard.stats.reviewTasks}
			color="yellow"
		></StatLabel>
		<StatLabel
			d="m9.75 9.75 4.5 4.5m0-4.5-4.5 4.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"
			head="Cancelled"
			value={$taskStore.dashboard.stats.cancelledTasks}
			color="red"
		></StatLabel>
	</div>
	...
	<div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-100">
		...
		<div class="p-4 sm:p-6">
			...
			{:else if $taskStore.dashboard.recentTasks.length === 0}
			...
			{:else}
				<div class="space-y-4">
					{#each $taskStore.dashboard.recentTasks as task (task.id)}
						<RecentTaskBlock
							taskId={task.id}
